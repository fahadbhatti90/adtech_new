...
export function getCampaignsPortfolioCall(profileId, adType, portfolioCampaign, callback, errorcallback) {
    axios.get(portfolioCampaignUrl, {
        ...
        if (callback != null) {
            let campaignOptions = res.data.data.map((obj, idx) => {
                return {
                    label: (obj.name),
                    value: obj.id,
                    key: idx
 ...
 export function savePresetRule(params,callback, errorcallback) {
    axios.post(SAVE_PRESET_RULE,params).then(res => {
        if (res.data.status) {
            ...
            let presetOptions = presetRulesMaping(res.data.rules);
            callback(res.data.message,presetOptions);
        }
        else{
...
export function getPesetRuleValue(params, callback, errorcallback) {
    axios.get(presetRuleUrl, {
        ...
        .then(res => {
            if(res.data.status){
                let data = helperSetPresetRuleValue(res.data.data[0]);
...
function helperSetPresetRuleValue(presetRule){
    let orignal = {
        selectedRulename: presetRule.presetName,
        selectedProfile: null,
        selectedAdType: null,
        selectedPortfolioCampaignType: null,
        selectedCampaign: null,
        selectedPreset: null,
        selectedPeriods: {label: "Last 7 days", value: presetRule.lookBackPeriod, className: 'custom-class'},
