...
import {getProfiles, getCampaigns, updateBudgetRule, getRecommendedEvents} from "../apiCalls";
...
class EditBudgetRuleForm extends Component {
    ...
    componentDidMount() {
        ...
        let params = {
            "id": row.id,
            "campaignType": adTypeOptions[0].value,
            "fkProfileId": row.fkProfileId,
        }

        getProfiles(params, ({profileOptions, CampaignsOptions}) => {
            let selCampaigns = row.budget_rule_campaigns;
            let selectedCampaigns = selCampaigns.map((obj, idx) => {
                return {
                    value: obj.id,
                    label: obj.name,
            ...
            let startD = moment(row.startDate).format("MM-DD-YYYY");
            if (startD <= currentD) {
                this.setState({
            ...
            this.setState({
                ...
                CampaignsOptions,
                selectedCampaigns,
                tempCampaignsOptions: CampaignsOptions,
                tempCampaigns: selectedCampaigns,
                dailyWeekly: row.recurrence,
                selectedProfile: profileOptions.filter(item => {
                    return breakProfileId(item.value) == row.fkProfileId
                })[0],
                ruleName: row.ruleName,
                selectedAdType: {
                    value: row.adType,
                    label: (row.adType != 'sponsoredBrands') ? 'Product' : 'Brand',
                    selected: 'selected'
                },
                selectedRuleType: {
                    value: row.ruleType,
                ...
                endDateDP: (row.endDate != null) ? moment(row.endDate).format("MM-DD-YYYY") : '',
                mon: (row.mon == 1),
                tue: (row.tue == 1),
                wed: (row.wed == 1),
                thu: (row.thu == 1),
                ...
                sat: (row.sat == 1),
                sun: (row.sun == 1),
                raiseBudget: row.raiseBudget,
            }, () => {
                if (row.ruleType == performanceBase.value){
                    this.setState({

                        selectedMetric: {
                            value: row.metric,
                            label: row.metric.toUpperCase(),
                            selected: 'selected'
                        },
                        selectedCondition: {
                            value: row.comparisonOperator,
                            label: row.comparisonOperator.replaceAll("_", " "),
                            selected: 'selected'
                        },
    ...
    makeRemovableCampaigns = (temp, actualValue) => {
        ...
        if (temp) {
            ...
            this.setState({
                removeCampaigns: getRemovedValues
            })
        }
    }
    onCampaignsChange = (value) => {
        ...
        let result = this.checkIfDataIsAvailable(this.state.tempCampaigns, value);
        ...
        this.setState({
            ...
            this.makeRemovableCampaigns(this.state.tempCampaigns, value)
        })
    ...
    updateStates = () => {
        this.setState((prevState) => ({
            ...prevState.errors,
            thresholdValueE: 'Raise budget must not be greater than 100.00'
    ...
    onSubmit = (event) => {
        ...
        if (Object.size(validationFormData) > 0 || isDateLess) {
            ...
            let thresholdValue = this.state.thresholdValue;
            if (thresholdValue !="" && parseFloat(thresholdValue) > 100.00) {

                this.setState((prevState) => {
                    return ({
                        errors: {
                            ...prevState.errors,
                            thresholdValueE: 'Threshold value not be greater than 100.00'
                        }
                    })
                }, () => {
                })
            ...
            let removeCampaigns = this.state.removeCampaigns;
            ...
            let params = {
                ...
                'removeCampaigns': removeCampaigns.map(item => {
                    return item.value;
                }),
            ...
            updateBudgetRule(params, (data) => {
                if (data.status) {
                    this.props.handleModalClose();
                    this.props.dispatch(ShowSuccessMsg(data.success, "Successfully", true, "", this.props.updateDataTableAfterSubmit()));
                } else {
                    this.setState({isProcessing: false});
                ...
                <div className="p-5 rounded-lg">
                    <form>
                        <Grid container spacing={1}>
                            <Grid container spacing={1} alignItems="flex-start" className="px-2 py-0 rounded-lg">
                                ...
                                <Grid item xs={12} sm={6} md={6} lg={4} className="autoScrl">
                                    <label className="text-sm  ml-2">
                                        ...
                                        {/*{*/}
                                        {/*    this.state.selectedRuleType.value == scheduleBase.value ?*/}
                                        {/*        <PrimaryButton*/}
                                    ...
                                    <MultiSelect
                                        ...
                                        customclassname="mr-5 ThemeSelect"
                                        isDisabled={(this.state.eventName != null)}
                                    />
                                ...
                                <Grid item xs={12} sm={6} md={6} lg={4}>
                                    ...
                                    <div
                                        onClick={this.state.eventName == null && this.state.isStartDateDisable ? this.handleOnStartDateRangeClick : ''}>
                                        <div className="ThemeInput mr-5">
                                            <TextFieldInput
                                ...
                                <Grid item xs={12} sm={6} md={6} lg={4}>
                                    ...
                                    <div onClick={this.state.eventName == null ? this.handleOnEndDateRangeClick : null}>
                                        <div className="ThemeInput mr-5">
                                            <TextFieldInput
                                ...
                                {this.state.dailyWeekly == recurrence.WEEKLY ?
                                    <Grid item xs={12} sm={5} md={5} lg={5}
                                        ...
                                        <CheckBox
                                            ...
                                            name="sun"
                                        />
                                    </Grid> : this.state.eventName != null ?
                                        <>
                            ...
                            {
                                this.state.selectedRuleType.value == performanceBase.value ?
...
export default (connect(null)(EditBudgetRuleForm))
