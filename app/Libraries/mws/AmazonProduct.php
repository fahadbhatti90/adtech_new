...
namespace App\Libraries\mws;
//use Sonnenglas\AmazonMws\AmazonProductsCore;
use App\Libraries\mws\AmazonProductsCore;
...
class AmazonProduct extends AmazonProductsCore
{
    ...
    public function loadXML($xml)
    {
       ...
       /* if ($xml->Relationships) {
        ...
        if ($xml->Relationships) {
            foreach ($xml->Relationships->children('',true) as $x) {
                foreach ($x->children('',true) as $z) {
                    foreach ($z->children('',true) as $zz) {
                        foreach ($zz->children('',true) as $zzz) {
                            $this->data['Relationships'][$x->getName()][$z->getName()][$zz->getName()][$zzz->getName()] = (string)$zzz;
                        }
                    }
                }
            }
        ...
        if ($xml->SalesRankings) {
            $this->data['SalesRankings']=$xml->SalesRankings;
        }
