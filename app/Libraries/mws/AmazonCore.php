<?php namespace App\Libraries\mws;
  ...
  <RequestID>123</RequestID>
    ...
    protected function checkResponse($r)
    {
        ...
        if ($r['code'] == 200) {
            ...
            $failed_array['errorCode']=(string)isset($xml->Code) && !empty($xml->Code) ? $xml->Code : 'NA';
            $failed_array['httpStatusCode']=(string)isset($r['code']) && !empty($r['code']) ? $r['code'] : 'NA';
            $failed_array['description']=(string)isset($xml->Message) && !empty($xml->Message) ? $xml->Message : 'NA';
            $failed_array['apiType']=(string)isset($this->options['Action']) && !empty($this->options['Action']) ? $this->options['Action'] : 'NA';
            $failed_array['sellerId']=(string)isset($this->options['SellerId']) && !empty($this->options['SellerId']) ? $this->options['SellerId'] : 'NA';
            $failed_array['reportType']=(string)isset($this->options['ReportType']) && !empty($this->options['ReportType']) ? $this->options['ReportType'] : 'NA';
            $failed_array['reportId']=(string)isset($this->options['ReportId']) && !empty($this->options['ReportId']) ? $this->options['ReportId'] : 'NA';
            $failed_array['reportRequestId']=(string)isset($this->options['ReportRequestIdList.Id.1']) && !empty($this->options['ReportRequestIdList.Id.1']) ? $this->options['ReportRequestIdList.Id.1'] : 'NA';
            $failed_array['startDate']=(string)isset($this->options['StartDate']) && !empty($this->options['StartDate']) ? $this->options['StartDate'] : 'NA';
            $failed_array['endDate']=(string)isset($this->options['EndDate']) && !empty($this->options['EndDate']) ? $this->options['EndDate'] : 'NA';

            $result_insert_failed__report=MWSModel::insert_failed_job($failed_array);
            if ($result_insert_failed__report){
                return false;
            }else{
                return false;
            }
    ...
    protected function sendRequest($url, $param)
    {
        ...
        while ($response['code'] == '503' && $this->throttleStop == false) {
            //$this->sleep();
            $this->log("request throtled sleeping for 5 seconds start");
            //$this->sleep();
            sleep(5);
            $this->log("request throtled sleeping for 5 seconds ends");
            $response = $this->fetchURL($url, $param);
        }
