...
class AmazonReport extends AmazonReportsCore
{
    ...
    {
        $this->log("RReport handler!", 'Warning');
        ...
        if ($this->mockMode) {
            ...
            $this->reportdata = $this->fetchMockFile(false);
        } else {
    ...
    public function saveReport($path)
    {
        ...
        try {
            $linesArray = array_filter(preg_split('/\R/', $this->reportdata));
            $report_data=array();
            foreach($linesArray as $line) {
                $report_data[] = explode("\t", $line);
            }
            $report_indexes_raw = $report_data[0];
            foreach ($report_indexes_raw as $value){
                $report_indexes[]=trim(str_replace(' ', '', $value));
            }
            //$report_indexes = $report_data[0];
            unset($report_data[0]);
            $store_report=array();
            $finalarray = array();
            for ($i=1; $i<= sizeof($report_data); $i++){
                for ($j=0; $j< sizeof($report_data[$i]); $j++) {
                    $finalarray[$report_indexes[$j]] = $report_data[$i][$j];
                }
                $store_report[]=$finalarray;
            }
            $this->log("Successfully saved report #" . $this->options['ReportId']);
            return $store_report;
        } catch (Exception $e) {

            $this->log("Unable to save report #" . $this->options['ReportId'] . " : $e", 'Urgent');
