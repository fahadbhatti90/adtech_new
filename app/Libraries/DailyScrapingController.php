...
class DailyScrapingController
{
    ...
    public static function getValidDepartmentId(){
        ...
        // return $Crawlers;
        ActivityTrackerModel::setActivity("Varification Result Positive Valid Cron Found","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        ...
        foreach ($Crawlers as $Crawler) {
         ...
         ActivityTrackerModel::setActivity("checkAvailableSchedules ","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
         return $crawler_id;
    }//end function
    ...
    public function ScrapData($threadName = null)
    {
        if($threadName == null )
        return "No Thread Name";
       
        $scrap_model= new ScrapModel();
        $asins = asinModel::with("getAsinAccounts:fkAccountId,fkAsinId")->where("allocatedThread",$threadName)->where("asin_status","1");
        
        if (!$asins->exists()) {
            // No Assins Found

           
            ActivityTrackerModel::setActivity("No Assin Found","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
            return "0";
        }
        ...
        ActivityTrackerModel::setActivity("Assin Found","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        /**
         * Replace 
         ...
         * 
         */
        $asins = $asins->with('collection.asin_cron')->get();
        ...
        ActivityTrackerModel::setActivity((json_encode(($stat))),"data","DailyScrapingController","app\Console\ScrapperCommands\Commands",date('Y-m-d H:i:s'));
        ...
        foreach ($asins as $key =>$a){
            ...
            //  $this->Scraper($a->asin_code);
            //starting scraping Process

            //test this first
            $crawler_id = $a->collection->asin_cron->id;
            $c_id = $a->c_id;
            try {
                $temp = $this->Scraper($a, $c_id, $crawler_id, $threadName);
                if(isset($temp["error_code"])){
                    $status = $temp['error_code'];
                    continue;
                }
                // array_push($scrapedResult,$temp);
                $scrap_model->InsertResults($temp);
                $status = ScraperConstant::ASIN_STATUS_COMPLETED;
                ...
                $err = array(
                    "id"=>$a->asin_id,
                    "asin"=>$a->asin_code,
                    "CollectionId"=>isset($a->collection->id) ? $a->collection->id:0,
                    "CollectionName"=>isset($a->collection->c_name) ? $a->collection->c_name:0,
                    "CollectionType"=>"Daily"
                );
                $failReasons = array(
                    str_limit($th->getMessage(), 200),
                );
                $crawler_id = isset($a->collection->asin_cron) ? $a->collection->asin_cron->id:0;
                  ...
                  $this->_set_fail_status(
                       ...
                       $crawler_id);
                Log::error("filePath:app\Console\Commands\ScrapperCommands\ScheduleCronCommand:cron Their's been an error while scraping Error Message (Fail Status Saved in DB) ".($th->getMessage()));
               
                ActivityTrackerModel::setActivity(" Error Message from catch (Fail Status Saved in DB)". str_limit($th->getMessage(), 300),"Error","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
    ...
    public  function testScrap($c){
        $asinModle = asinModel::first();
        return $this->Scraper($asinModle,$c,0,"t1");
    }
    ...
    private function Scraper($asin,$c_id, $crawler_id = 0, $tName)
    {
        $url = "https://www.amazon.com/dp/".$asin->asin_code;
        // requesting for scraped Data
       
        $errorCode = ScraperConstant::ASIN_STATUS_COMPLETED;
        $asin_data = $this->get_data_scraped($url,$c_id,$asin->asin_code, $crawler_id, $tName);
        ...
        elseif ($asin_data['error_code'] == ScraperConstant::ERROR_CAPTCHA) {
            ...
            unset($asin_data);
            ActivityTrackerModel::setActivity("From Error Check".$data['error_text'] ,"Constant ERROR","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        ...
        elseif ($asin_data['error_code'] == ScraperConstant::ERROR_PRODUCTS_NOT_FOUND) {
            ...
            //$fkAccountIds = $asin->getAccountIds();
    ...
    private function _filterDimension($dimensions,$type = "d"){
        ...
        {
            if(isset($dbreaked[0]))
            $data["length"] = isset($dbreaked[0]) ? preg_replace("/[^a-zA-Z0-9x.,_-]+/", "", trim($dbreaked[0])) : "NA";
            $data["width"] = isset($dbreaked[1]) ? trim($dbreaked[1]) : "NA";
            $data["height"] = isset($dbreaked[2]) ? trim($dbreaked[2]) : "NA";
            $data["height"] = preg_replace("/[^0-9.,_-]+/", "", $data["height"]);
            $data["unit"] =  isset($dbreaked[2]) ? preg_replace("/[^a-zA-Z]+/", "", $dbreaked[2]): "NA";
            if($data["unit"] != "NA"){
...
END;
    ...
    public function  get_data_scraped($url, $c_id=null, $asin=null, $crawler_id = 0, $tName){
        $data = NULL;
        $tries = 0;
        //First Try
   
        ActivityTrackerModel::setActivity(" First Try for scraped Data ","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
          
        do{

            $data = $this->ASINScrapingCustom($url,$c_id,$asin, $tName);
           
                if($data['status'] == FALSE){     
                    
             
     
                ActivityTrackerModel::setActivity($tries." try for scraped Data ","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
                $tries++;
                }else{
                    //ASIN Successfully Scraped Breaking Loop
                  
        
                    ActivityTrackerModel::setActivity("Successfully Scraped Breaking Loop","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
    ...
    public function  get_search_rank_data_scraped($url, $c_id=null, $asin=null, $crawler_id = 0){
        $data = NULL;
        $tries = 0;
        //First Try
        ...
        do{
            ...
            $data = $this->searchRankScrapingCustom($url,$c_id,$asin);
    ...
    public function initProxy(){
        ...
        else
        {
            ...
            ActivityTrackerModel::setActivity(" No Proxy Avalialble","proxyError","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
    ...
    public function get_data_curl($url,$c_id=null,$asin=null)
    {
        ...
        ActivityTrackerModel::setActivity(" Getting proxy ","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        ...
        ActivityTrackerModel::setActivity("  Initializing Curl ","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
       ...
       $headers = [];
        ...
        ActivityTrackerModel::setActivity("Execution Curl","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        ...
        $return['http_code'] = curl_getinfo($ch, CURLINFO_HTTP_CODE);
         if (curl_errno($ch)) {
            ...
            ActivityTrackerModel::setActivity("  Error Found Line Number 860 HTTP CODE:" . str_limit(($return['error_text']), 500) ,"Error","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
        }
        ...
        if (is_null($return['error_text'])) {
            ...
            if ($this->check_captcha($data) == ScraperConstant::ERROR_CAPTCHA) {
                ...
                ActivityTrackerModel::setActivity(
                    "Capcha Found ",
                    "Captcha",
                    "DailyScrapingController",
                    "App\Libraries\DailyScrapingController",
                    date('Y-m-d H:i:s')
                );
                $dom = new DOMDocument();
                libxml_use_internal_errors(true);
                ...
                $capctha = $this->solve_capcha($capcha_image,$c_id);
                 if ($capctha['status'] == TRUE) {
                   ...
                   ActivityTrackerModel::setActivity("Capcha Not Guessed Line Number 705 ".str_limit(json_encode($return) , 500),"info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
                }
                ...
                if ($v_result["status"]) {
                    ...
                    ActivityTrackerModel::setActivity("Data is Valid status true","info","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
                    ...
                    // ActivityTrackerModel::setActivity($v_result["error_code"]." Error","Error","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
                }
    ...
    private function _set_fail_status($a,$url,$status,$crawler_id= null){
        //foreach on accountIds of asin failed and create MultidimensionalArray for insertion in fail status table
        //change the add recored function from create to insert
        //This might slow the whole process
        $accounts = $a->getAsinAccounts;
        $fData = [];
        ...
        do {
            ...
            if (curl_errno($ch)) {
                ...
                if(str_contains(curl_error($ch), "HTTP code 407 from proxy") || str_contains(curl_error($ch), "Failed to connect to")){
                    $proxy_row->is_blocked = 1;
                    $proxy_row->save();
                    ActivityTrackerModel::setActivity("Ip Block $proxy_row->proxy_ip $Ips", "ipBlocked", "DailyScrapingController", "App\Libraries\DailyScrapingController", date('Y-m-d H:i:s'));
                }
            }
            ...
            if (is_null($return['error_text'])) {
                ...
                if ($this->check_captcha($data) == ScraperConstant::ERROR_CAPTCHA) {
                    ...
                    // echo "<br>";
                    ActivityTrackerModel::setActivity("Captcha Found Retrying  $proxy_row->proxy_ip $Ips", "Captcha D", "DailyScrapingController", "App\Libraries\DailyScrapingController", date('Y-m-d H:i:s'));
                    $isCaptchaFound = true;
                    ...
                    if ($v_result["status"]) {
                        // Data is Valid status true
                        ActivityTrackerModel::setActivity("Success $proxy_row->proxy_ip $Ips", "Success", "DailyScrapingController", "App\Libraries\DailyScrapingController", date('Y-m-d H:i:s'));
                        $return['data'] = $data;
                        $return['status'] = TRUE;
                        break;
                    } else {
                        ...
                        $return['data'] = NULL;
                        $return['status'] = FALSE;
                        ActivityTrackerModel::setActivity("Error Occureds $proxy_row->proxy_ip $Ips".$return['error_code'], "Captcha D", "DailyScrapingController", "App\Libraries\DailyScrapingController", date('Y-m-d H:i:s'));
                        // $isCaptchaFound = true;
                        $proxy_rowt = $this->initProxy();
                        $proxy_row->is_blocked = 0;
                        $proxy_row->save();
    ...
    public function searchRankScrapingCustom($url,$c_id=null,$asin=null)
    {
        ...
        do {
            ...
            if (is_null($return['error_text'])) {
                ...
                if ($this->check_captcha($data) == ScraperConstant::ERROR_CAPTCHA) {
                    ...
                    // print_r("$url Captcha Found");
                    // echo "<br>";
                    ActivityTrackerModel::setActivity("Captcha Found Retrying","Captcha D","DailyScrapingController","App\Libraries\DailyScrapingController",date('Y-m-d H:i:s'));
