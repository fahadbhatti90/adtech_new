...
class UpdateTargetBid extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'updateTargetbid:tacos {data*}';
     ...
     * @var string
     */
    protected $description = 'This command is use for update the tacos bidding rule value of specific Target SD.';
    ...
    {
        Log::info("App\Console\Commands\Tacos\UpdateTargetBid. Start Cron.");
        ...
        if (env('APP_ENV') == 'production') {
            ...
            if (!empty($dataArgumants)) {
                ...
                $url = Config::get('constants.amsApiUrl') . '/' . Config::get('constants.sdTargetUpdateBid');
                $jsonArrayObj = (object)[
                    'targetId' => $dataArgumants['targetId'],
                ...
                if (!empty($jsonArray)) {
                    ...
                    if (!empty($accessToken)) {
                        try {
                            ...
                            $try += 1;
                            if ($try >= 3) {
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid profile id:' . $profileId . ' and number try is: ' . $try . ' error code :' . $ex->getCode(), 'success');
                                return false;
                            }
                            ...
                            if ($errorCode == 401) {
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid 503. profile id:' . $profileId . ' and number try is: ' . $try, '503');
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '400');
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '403');
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '404');
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '422');
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '500');
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                                ...
                                AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, $errorCode);
                                sleep(Config::get('constants.sleepTime') + 2);
                                if ($try == 1) {
                                    $addNotification->addAlertNotification($notificationData);
                                }
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('App\Console\Commands\Tacos\UpdateTargetBid and error code :' . $ex->getCode(), 'fail');
                            // store report status
                ...
                Log::info("Data argument not found.");
            }
        }
        Log::info("filePath:App\Console\Commands\Tacos\UpdateTargetBid. End Cron.");
    }
    ...
    private function notificationData($data, $errorCode)
    {
        ...
        $state = "Update call for target bid";
        $notificationData = [];
