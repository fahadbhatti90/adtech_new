...
namespace App\Console\Commands\Mws\runGetAsinsFromReportsCron;
...
class runGetAsinsFromReportsCron extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'runGetAsinsFromReportsCron:cron';
    ...
    public function handle()
    {
        ...
        if ($active_crons) {
            foreach ($active_crons as $value) {
                if ($current_date != $value->getAsinsFromReportsLastRun){
                    $cron_time = DateTime::createFromFormat('H:i', $value->getAsinsFromReportsTime);
                    ...
                    if ($cron_time >= $start && $cron_time <= $end)
                    {
                        ...
                        if ($value->report_type=='Catalog'){
                            ...
                            $data['getAsinsFromReportsCompletedTime']=date('Y-m-d H:i:s');
                            $data['getAsinsFromReportsLastRun']=$current_date;
