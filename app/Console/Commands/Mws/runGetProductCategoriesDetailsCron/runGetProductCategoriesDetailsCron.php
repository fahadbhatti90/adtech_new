...
namespace App\Console\Commands\Mws\runGetProductCategoriesDetailsCron;
...
class runGetProductCategoriesDetailsCron extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'runGetProductCategoriesDetailsCron:cron';
    ...
    public function handle()
    {
        ...
        if ($active_crons) {
            foreach ($active_crons as $value) {
                if ($current_date != $value->getProductCategoriesDetailsLastRun){
                    $cron_time = DateTime::createFromFormat('H:i', $value->getProductCategoriesDetailsTime);
                    ...
                    if ($cron_time >= $start && $cron_time <= $end)
                    {
                        ...
                        if ($value->report_type=='Catalog'){
                            ...
                            $data['getProductCategoriesDetailsCompletedTime']=date('Y-m-d H:i:s');
                            $data['getProductCategoriesDetailsLastRun']=$current_date;
