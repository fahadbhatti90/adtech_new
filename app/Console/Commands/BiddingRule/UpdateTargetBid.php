...
class UpdateTargetBid extends Command
{
     ...
     * @var string
     */
    protected $signature = 'updateTargetbid:updatebid {data*}';
     ...
     * @var string
     */
    protected $description = 'This command is use for update the bidding rule value of specific Target SD.';
    ...
    {
        Log::info("App\Console\Commands\BiddingRule\UpdateTargetBid. Start Cron.");
                    ...
                    if (!empty($jsonArray)) {
                        ...
                        if (!empty($accessToken)) {
                            try {
                                ...
                                $try += 1;
                                if ($try >= 3) {
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid profile id:' . $profileId . ' and number try is: ' . $try . ' error code :' . $ex->getCode(), 'success');
                                    return FALSE;
                                }
                                ...
                                if ($errorCode == 401) {
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '503');
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '400');
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '403');
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '404');
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '422');
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, '500');
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
                                    ...
                                    AMSModel::insertTrackRecord('BiddingRule\UpdateTargetBid. profile id:' . $profileId . ' and number try is: ' . $try, $errorCode);
                                    sleep(Config::get('constants.sleepTime') + 2);
                                    if ($try == 1) {
                                        $addNotification->addAlertNotification($notificationData);
                                    }
        ...
        Log::info("filePath:App\Console\Commands\BiddingRule\UpdateTargetBid. End Cron.");
    }
