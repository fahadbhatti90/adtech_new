...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getcampaignreportlink:sdcampaign {day?}';
    ...
    public function handle()
    {
        ...
        if ($AllProfileID->isNotEmpty()) {
            foreach ($AllProfileID as $single) {
                ...
                $existReport = CampaignSDModel::where('profileID', $single->getReportId->profileID)
                ...
                try {
                    ...
                    $response = $client->request('GET', $url . '/' . $single->getReportId->reportId, [
                        'headers' => [
                            ...
                            'Amazon-Advertising-API-Scope' => $single->getReportId->profileID],
                    ...
                    if (!empty($body) && $body != null) {
                        $reportId = ReportIdModel::find($single->getReportId->id);
                        $reportId->profileID = $single->getReportId->profileID;
                        ...
                        switch ($reportLinkStatus) {
                            case "FAILURE":
                                ...
                                break;
                            default:
                                $reportLink = new CampaignSDModel();
                        ...
                        $reportLink->profileID = $single->getReportId->profileID;
