...
use App\Models\ams\Report\Data\Campaign\SD\CampaignSDDataModel;
                    ...
                    if (!empty($accessToken)) {
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $reportLink = CampaignSDModel::find($single->id);
                                ...
                                for ($i = 0; $i < $totalNumberOfRecords; $i++) {
                                    $reprotData = new CampaignSDDataModel();
                            ...
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    if (strstr($ex->getMessage(), 'Not authorized to access scope')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\Campaign\SD\getReportLinkDataCron. End Cron.');
