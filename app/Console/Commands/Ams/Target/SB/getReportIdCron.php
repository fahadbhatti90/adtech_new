...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportid:sbtargets {day?}';
    ...
    public function handle()
    {
        ...
        if ($AllProfileID->isNotEmpty()) {
            ...
            foreach ($AllProfileID as $single) {
                ...
                } elseif ($existReportBatch['status'] == 5 || $existReportBatch['status'] == 7 || $existReportBatch['status'] == 8 || $existReportBatch['status'] == 9 ) {
                    profileReportStatus::where('id', $existReportBatch['id'])
                        ->update(['status' => 0, 'error_description' => 'NA']);
                } elseif ($existReportBatch['status'] != 0) {
                    continue;
                }
                b:
