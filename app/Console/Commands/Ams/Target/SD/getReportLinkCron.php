...
use App\Models\ams\Report\Link\ProductsAds\SP\ProductsAdsSPModel;
use App\Models\ams\Report\Link\Target\SD\TargetsSDModel;
use App\Models\ams\Report\Link\Target\SP\TargetAdsSPModel;
...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportlink:sdtargets {day?}';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Target SD report link.';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SD\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        $reportType = 'Product_Targeting_SD';
        ...
        if ($AllProfileID->isNotEmpty()) {
            foreach ($AllProfileID as $single) {
                ...
                $existReport = TargetsSDModel::where('profileID', $profileID)
                ...
                try {
                    ...
                    if (!empty($body) && $body != null) {
                        ...
                        $reportLink->save();
                        // store report status
                        AMSModel::insertTrackRecord('report name : Target SD Report Link' . ' profile id: ' . $single->profileId, 'record found');
                    } else {
                        // store report status
                        AMSModel::insertTrackRecord('report name : Target SD Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                    }
                } catch (\Exception $ex) {
                    if ($ex->getCode() == 401) {
                    ...
                    // store report status
                    AMSModel::insertTrackRecord('Target SD Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\Target\SD\getReportLinkCron. End Cron.");
    }
}
