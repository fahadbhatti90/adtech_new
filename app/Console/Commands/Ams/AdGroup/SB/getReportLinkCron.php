...
use App\Models\ams\Report\Link\Adgroup\SB\AdgroupSBModel;
...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsbadgroupreportlink:sbadgroup {day?}';
    ...
    public function handle()
    {
        ...
        if ($AllProfileID->isNotEmpty()) {
            foreach ($AllProfileID as $single) {
                ...
                $existReport = AdgroupSBModel::where('profileID', $profileID)
                ...
                try {
                    ...
                    if (!empty($body) && $body != null) {
                        ...
                        $reportLink->save();
                        // store report status
                        AMSModel::insertTrackRecord('report name : AdGroup Report Link' . ' profile id: ' . $single->getReportId->profileID, 'record found');
                    } else {
                        // store report status
                        AMSModel::insertTrackRecord('report name : AdGroup Report Link' . ' profile id: ' . $single->getReportId->profileID, 'not record found');
                    }
                } catch (\Exception $ex) {
                    if ($ex->getCode() == 401) {
