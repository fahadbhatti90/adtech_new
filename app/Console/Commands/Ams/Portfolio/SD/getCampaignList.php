...
class getCampaignList extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSandBoxSDCampaignlist:portfolio';
     ...
     * @var string
     */
    protected $description = 'This Command is used to get Day partying Sponsored Display Campaign List';
        ...
        if ($allProfileIds->isNotEmpty()) {
            ...
            foreach ($allProfileIds as $single) {
                ...
                try {
                    ...
                    $try += 1;
                    if ($try >= 3) {
                        AMSModel::insertTrackRecord('SD Day partying profile id:' . $profileId . ' and number try is: ' . $try.' error code :'.$ex->getCode(), 'success');
                        $try = 0;
                        continue;
                    }
                    if ($ex->getCode() == 401) {
                        ...
                        AMSModel::insertTrackRecord('SD day partying getCampaignList 503. profile id:' . $profileId . ' and number try is: ' . $try, '503');
                        sleep(Config::get('constants.sleepTime') + 5);
                        goto b;
                    }
                    // store report status
                    AMSModel::insertTrackRecord('App\Console\Commands\Ams\Portfolio\SD\getSDCampaignlist and error code :'.$ex->getCode(), 'fail');
