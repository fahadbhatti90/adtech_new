...
namespace App\Console\Commands\Ams\Historical\Target\SB;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargethistoricalreportid:sbtargets {profile*}';
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->SponsoredBrandTargetingSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'SponsoredBrandTargetingSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            a:
