...
namespace App\Console\Commands\Ams\Historical\Keyword\SB;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getkeywordhistoricalreportid:sbkeyword {profile*}';
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->keywordSbSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'keywordSbSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            a:
