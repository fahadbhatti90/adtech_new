...
namespace App\Console\Commands\Ams\Historical\ProductsAds;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getproductsadshistoricalreportid:productsads {profile*}';
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->productAdsSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'productAdsSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            a:
