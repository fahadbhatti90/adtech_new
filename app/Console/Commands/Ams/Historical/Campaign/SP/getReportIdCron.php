...
namespace App\Console\Commands\Ams\Historical\Campaign\SP;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getcampaignhistoricalreportid:spcampaign {profile*}';
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->campaignSpSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'campaignSpSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            a:
