...
namespace App\Console\Commands\Ams\Historical\AdGroup\SP;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getadgrouphistoricalreportid:spadgroup {profile*}';
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->adGroupSpSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'adGroupSpSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            a:
