...
namespace App\Console\Commands\Ams\Historical\AdGroup\SD;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsdadgrouphistoricalreportid:sdadgroup {profile*}';
            ...
            foreach ($AllProfileID as $single) {
                ...
                if ($single->marketplaceStringId != 'A1AM78C64UM0Y8') {
                    ...
                    if (!empty($clientId)) {
                        ...
                        $client = new Client();
                        if ($single->SponsoredDisplayAdgroupSixtyDays == 0) { // check Sixty Days is inserted OR not
                            AMSModel::UpdateProfileSixtyStatus($single->profileId, 'SponsoredDisplayAdgroupSixtyDays', 1);
                            for ($i = 60; $i >= 1; $i--) {
                                a:
