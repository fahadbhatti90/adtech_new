...
namespace App\Console\Commands\Ams\Historical\AdGroup\SB;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsbadgrouphistoricalreportid:sbadgroup {profile*}';
    ...
    public function handle()
    {
        ...
        Log::info("filePath:App\Console\Commands\Ams\Historical\AdGroup\SB. Start Cron.");
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $client = new Client();
                    if ($single->SponsoredBrandAdgroupSixtyDays == 0) { // check Sixty Days is inserted OR not
                        AMSModel::UpdateProfileSixtyStatus($single->profileId, 'SponsoredBrandAdgroupSixtyDays', 1);
                        for ($i = 60; $i >= 1; $i--) {
                            // get account id from
                            a:
